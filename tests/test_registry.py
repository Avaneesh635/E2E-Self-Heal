...
def test_aggregate_failure_stats_deterministic_order():
    ...
    records1 = [
        HealingRecord(
            component="B",
            original_selector="#old-b",
            replacement_selector="#new-b",
            reason="button id was renamed",
            test_script_path="a",
        ),
        HealingRecord(
            ...
            original_selector=".old-b",
            replacement_selector=".new-b",
            reason="className changed",
            test_script_path="a",
        ),
        HealingRecord(
            component="A",
            original_selector="#old-a",
            replacement_selector="#new-a",
            reason="button id was renamed",
            test_script_path="a",
        ),
        HealingRecord(
            component="A",
            original_selector="[data-testid='a']",
            replacement_selector="role=button[name='A']",
            reason="className changed",
            test_script_path="a",
        ),
    ]
    ...
    assert list(stats1.components) == ["A", "B"]
    assert list(stats2.components) == ["A", "B"]
